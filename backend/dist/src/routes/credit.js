...
const esewaParser_1 = require("../services/esewaParser");
...
router.post("/upload", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        // Step 1: Parse PDF
        const esewaProfile = await (0, esewaParser_1.parseEsewaPdf)(req.file.path);
        if (esewaProfile.txCount === 0) {
            res.status(422).json({
                error: "Could not extract transactions from PDF. Make sure this is an eSewa statement.",
            });
            return;
        }
        // Step 2: Compute credit score
        const scoreResult = (0, esewaParser_1.computeCreditScore)(esewaProfile);
        console.log(`📊 Credit score computed: ${scoreResult.score} for ${borrowerAddress}`);
        console.log(`   Breakdown:`, scoreResult.breakdown);
        // Step 3: Write to Solana
        const txSignature = await (0, solanaOracle_1.writeCreditScore)(borrowerAddress, scoreResult.score, esewaProfile.totalVolumeNPR, esewaProfile.txCount);
...
router.get("/:borrowerAddress", async (req, res) => {
    try {
        ...
        const profile = await (0, solanaOracle_1.fetchCreditProfile)(borrowerAddress);
        if (!profile) {
            res
                .status(404)
